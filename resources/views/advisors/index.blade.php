        ...
        <ul role="list"
          ...
          @foreach($advisors as $advisor)
            <li>
              ...
              <h3 class="mt-6 text-xl font-semibold leading-8 text-slate-600">{{ $advisor->name }}</h3>
              <p class="leading-7 text-slate-500">{{ $advisor->designation }}</p>
              <p class="text-sm leading-6 text-slate-500">{{ $advisor->organization }}</p>
            </li>
          @endforeach
          <!-- Advisor -->
        </ul>
