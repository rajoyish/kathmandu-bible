        ...
        @foreach($galleries as $gallery)
          <li>
            <a href="{{ route('galleries.show', $gallery) }}" class="block relative pt-2.5 hover:opacity-75">
              <span class="absolute inset-x-4 top-0 h-16 rounded-2xl bg-slate-300"></span>
              <span class="absolute inset-x-2 top-0 mt-1 h-16 rounded-2xl border-t-2 border-white bg-slate-400"></span>
              <img class="relative aspect-[3/2] w-full rounded-2xl border-t-2 border-white object-cover"
                   src="{{ url('storage/'.$gallery->cover_photo) }}" alt="Gallery of {{ $gallery->title }}"/>
